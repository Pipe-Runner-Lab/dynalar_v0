...
template<typename T>
inline T* PopulateNormals(T* vertices, GLuint* indices, unsigned int vertexCount, unsigned int layoutCount, unsigned int indexCount, unsigned int vertexOffsetCount, unsigned int normalOffsetCount)
{
	...
	for (int layoutIdx = 0; layoutIdx < vertexCount; layoutIdx += layoutCount) {
